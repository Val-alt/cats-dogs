...
var translationBreeds,engBreeds,itemId,itemObj,arrPreviousImage,dataPage=$("body").attr("data-page"),dataPageForJQ="[data-page='"+dataPage+"'] ",engCatBreeds={},engCatsObj=[],correctBreedDogTranslation={waterdog:"водяная собака",papillon:"папильён",african:"африканская",cattledog:"пастушья собака",cotondetulear:"котон-де-тулеар",entlebucher:"энтлебухер",eskimo:"эскимосская собака",germanshepherd:"немецкая овчарка",mexicanhairless:"ксолоитцкуинтли",mountain:"зенненхунд",pointer:"пойнтер",pug:"мопс",puggle:"пагль",redbone:"редбон кухаунд",shihtzu:"ши-тцу",stbernard:"сербернар"},correctBreedCatTranslation={Cheetoh:"чито"},App={options:{},init:function(){this.animateLine(),this.loadingBreeds(),this.btnLeftRight()},animateLine:()=>{let a,e;e="0px",$(window).width()>"1234"?a="600px":$(window).width()>"646"?a="280px":$(window).width()<="575"&&(a="200px"),$("[data-btn='main-page']").on("mouseenter",function(){$(this).find(".option__line").animate({width:a},250)}),$("[data-btn='main-page']").on("mouseleave",function(){$(this).find(".option__line").animate({width:"0px"},250)})},getKeyByValue:(a,e)=>Object.keys(a).find(t=>a[t]===e),loadingBreeds:()=>{"dogs"===dataPage&&$.getJSON("https://dog.ceo/api/breeds/list/all").then(a=>engBreeds=Object.keys(a.message)).then(a=>{translationBreeds=correctBreedDogTranslation;for(let e=0;e<a.length;e++){let t=a[e];translationBreeds[t]||App.translateBreeds(t,a)}}).catch(a=>{$(dataPageForJQ+"[data-card='content']").html("не удалось перевести породы собачек")}),"cats"===dataPage&&$.getJSON("https://api.thecatapi.com/v1/breeds").then(a=>{for(let e=0;e<a.length;e++)engCatBreeds[a[e].name]=a[e].id,engCatsObj[e]=a[e];return engCatBreeds}).then(a=>{translationBreeds=correctBreedCatTranslation;for(let e=0;e<Object.keys(a).length;e++){let t=Object.keys(a)[e];translationBreeds[t]||App.translateBreeds(t,a)}}).catch(a=>{$(dataPageForJQ+"[data-card='content']").html("не удалось перевести породы кошечек")})},translateBreeds:(a,e)=>{$.getJSON("https://translate.yandex.net/api/v1.5/tr.json/translate?key=trnsl.1.1.20190118T073029Z.3f310cb0cd1394d9.52268319f275afdd6bc274241581fc691aa64ccd&text="+a+"&lang=en-ru").then(t=>{let d=t.text[0].toLowerCase();if(translationBreeds[a]=d,Object.keys(translationBreeds).length===Object.keys(e).length){let a,e=Object.values(translationBreeds).sort(),t="";$(dataPageForJQ+"[data-icon='load-breed']").hide(),$(dataPageForJQ+".breeds-dropdown .dropdown-toggle").html('cписок пород <i class="fas fa-check ml-2" data-icon="check"></i>'),$(dataPageForJQ+"[data-icon='check']").fadeOut(600);for(let a of e)t+='<button class="dropdown-item" type="button" value="'+App.getKeyByValue(translationBreeds,a)+'" data-btn="breed-item">'+a+"</button>";$(dataPageForJQ+"[data-search='input']").prop("disabled",!1),$(dataPageForJQ+"[data-btn='search']").prop("disabled",!1),$(dataPageForJQ+".breeds-select").html(t),"cats"===dataPage&&(a=engCatBreeds),App.clickDropdown(a),App.searchBreed(e)}})},clickDropdown:a=>{$(dataPageForJQ+"[data-btn='breed-item']").on("click",function(){arrPreviousImage=[];let a=$(this).html(),e=$(this).val();"dogs"===dataPage&&(itemId=e),"cats"===dataPage&&(itemId=engCatBreeds[e],itemObj=engCatsObj.find(a=>a.name===e)),App.pictBreed(itemId,a,itemObj)})},pictBreed:(a,e,t)=>{let d;$(dataPageForJQ+"[data-icon='load-pict']").show(),"dogs"===dataPage?d="https://dog.ceo/api/breed/"+a+"/images/random":"cats"===dataPage&&(d="https://api.thecatapi.com/v1/images/search?breed_ids="+a),$.getJSON(d).then(a=>{let d;if($(dataPageForJQ+"[data-card='title']").html(""),$(dataPageForJQ+"[data-card='title']").html('Порода <span class="breed text-primary">'+e+"</span>"),"dogs"===dataPage?d=a.message:"cats"===dataPage&&(d=a[0].url),$(dataPageForJQ+"[data-icon='load-pict']").hide(),$(dataPageForJQ+"[data-card='content']").html('<img class="img-fluid" src="'+d+'">'),"cats"===dataPage){let a;for(var r in $("[data-characteristics]").show(),a=($(window).width(),22),t)"life_span"==r?$(dataPageForJQ+'div[data-param="'+r+'"]').html(t[r]):$(dataPageForJQ+'div[data-param="'+r+'"]').css("width",t[r]*a+"px")}arrPreviousImage.push($(dataPageForJQ+"[data-card='content']").html()),$(dataPageForJQ+"[data-btn='left-pict']").prop("disabled",!0),$(dataPageForJQ+"[data-btn='left-pict']").css("visibility","visible"),$(dataPageForJQ+"[data-btn='right-pict']").css("visibility","visible"),arrPreviousImage.length>1&&$(dataPageForJQ+"[data-btn='left-pict']").prop("disabled",!1)}).catch(()=>{$(dataPageForJQ+"[data-icon='load-pict']").hide(),$(dataPageForJQ+"[data-card='content']").html("не удалось загрузить картинку")})},btnLeftRight:()=>{$("[data-btn='right-pict']").on("click",()=>{let a=$(dataPageForJQ+".breed").html();if(void 0===itemId)return!1;App.pictBreed(itemId,a,itemObj)}),$("[data-btn='left-pict']").on("click",()=>{if(!(arrPreviousImage.length>1))return!1;$(dataPageForJQ+"[data-card='content']").html(""),$(dataPageForJQ+"[data-card='content']").html(arrPreviousImage[arrPreviousImage.length-2]),arrPreviousImage.pop(),arrPreviousImage.length<=1&&$(dataPageForJQ+"[data-btn='left-pict']").prop("disabled",!0)})},searchBreed:a=>{let e,t,d;$("[data-search='input']").keyup(function(r){r.preventDefault(),d=$(r.target).val().toLowerCase(),e=a,t="";for(let a of e){let e=a.substring(0,d.length);d===e&&""!=d&&(t+="<li class='search__item' data-search='item'>"+a+"</li>",$(dataPageForJQ+"[data-search='result']").css("display","block"),$(dataPageForJQ+"[data-search='list']").html(t),$("[data-search='item']").on("click",a=>{arrPreviousImage=[];let e=$(a.target).html(),t=App.getKeyByValue(translationBreeds,e);"dogs"===dataPage&&(itemId=t),"cats"===dataPage&&(itemId=engCatBreeds[t],itemObj=engCatsObj.find(a=>a.name===t)),App.pictBreed(itemId,e,itemObj),$(dataPageForJQ+"[data-search='input']").val("")}))}""==d&&(t="",$(dataPageForJQ+"[data-search='list']").html(t),$(dataPageForJQ+"[data-search='result']").css("display","none")),$(document).on("click",a=>{$(dataPageForJQ+"[data-search='input']").is(a.target)||(t="",$(dataPageForJQ+"[data-search='list']").html(t),$(dataPageForJQ+"[data-search='result']").css("display","none"))}),27==r.keyCode&&(this.value="",$(dataPageForJQ+"[data-search='result']").css("display","none"))}),$("[data-btn='search']").on("click",()=>{if(d.length<4)$("[data-search='input']")[0].value="",$(dataPageForJQ+"[data-search='result']").css("display","none"),alert("Введите четыре или более символов");else if(d.length>=4){if(/[a-z\s]/i.test(d))$("[data-search='input']")[0].value="",alert("Некорректный ввод");else if(App.getKeyByValue(translationBreeds,d)){arrPreviousImage=[];let a=App.getKeyByValue(translationBreeds,d),e=d;"dogs"===dataPage&&(itemId=a),"cats"===dataPage&&(itemId=engCatBreeds[a],itemObj=engCatsObj.find(e=>e.name===a)),App.pictBreed(itemId,e,itemObj),$("[data-search='input']")[0].value="",$(dataPageForJQ+"[data-search='result']").css("display","none")}else alert("Такой породы нет, попробуйте еще раз"),$("[data-search='input']")[0].value=""}})}};$(document).ready(function(){App.init()});